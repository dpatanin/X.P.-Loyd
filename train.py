...
tb_callback = keras.callbacks.TensorBoard(log_dir=config["log_dir"], update_freq=100)
...
def compile_and_fit(
    ...
    tb_callback.log_dir = (
        f"{config['log_dir']}/{config['description']}_{now}/{name}_{now}"
    )
    ...
    model.fit(
        wg.make_dataset(train or dp.train_df),
        epochs=config["episodes"],
        validation_data=wg.make_dataset(val or dp.val_df),
        callbacks=[tb_callback, early_stopping],
    )
    model.evaluate(
        wg.make_dataset(test or dp.test_df), verbose=0, callbacks=[tb_callback]
    )
