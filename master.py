...
param_test = [5, 15, 25, 50]

for param_case in param_test:
    ...
    for e in range(1, config["episodes"] + 1):
        ...
        trader.model.save(
            f"{config['model_directory']}/{config['model_name']}_{param_case}_{'ep{e}' if e == config['episodes'] else 'terminal'}_{now}.h5"
        )

    pbar.close()
    df = pd.DataFrame(profit_list)
    df.to_excel(f"data/training_{config['model_name']}_{param_case}_{now}.xlsx")
    ...
    # dp.step_size = 1
    trader.memory.clear()
    trader.load(f"{config['model_directory']}/{config['model_name']}_{param_case}_terminal_{now}.h5")
    ...
    for trial in range(1, 6):
        ...
        balance_list.to_excel(
            f"data/validation_{config['model_name']}_{param_case}_trial{trial}_{now}.xlsx"
        )
